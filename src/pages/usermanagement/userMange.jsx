...
const UserManagement = () => {
    ...
    const validateObj = userData && userData.permissions && userData.permissions.length > 0 && userData.permissions.filter(eachItem => eachItem.menu == "Users")
                ...
                <div className="d-flex">
                    ...
                    {subValDashboard && subValDashboard.edit && subValDashboard.edit.display === true && <>
                         {props.status === "ARCHIVE" ?   <a style={{opacity:"0.5"}} className="text-danger action-button"><i
                                        className="mdi mdi-delete font-size-18"></i></a> : <a onClick={e => handleDeleteUser(e, props.userid)}
                                    className={`${subValDashboard && subValDashboard.edit && subValDashboard.edit.enable === false ? 'pe-none' : ''} text-danger action-button`}><i
    ...
    const handleDelete = (e) => {
        setIsLoading(true)
        axios.delete(lambda + '/deleteuser?appname=' + appname + '&userid=' + deleteId + '&token=' + localStorage.getItem("token"))
            .then(response => {
